...
public class FluidIngredient implements Predicate<FluidStack> {
    ...
    private static final MapCodec<Value> CODEC_VALUE = NeoForgeExtraCodecs.xor(FluidValue.CODEC, TagValue.CODEC)
    ...
    public static FluidIngredient of(RegistryFriendlyByteBuf buff) {
        var type = buff.readByte();
        if (type == 0) {
            return new FluidIngredient(new FluidValue(FluidStack.STREAM_CODEC.decode(buff)));
        } else if (type == 1) {
    ...
    public void to(RegistryFriendlyByteBuf buff) {
        if (this.value instanceof FluidValue f) {
            buff.writeByte(0);
            FluidStack.STREAM_CODEC.encode(buff, f.fluid);
    ...
    public record FluidValue(FluidStack fluid) implements Value {
        public static final MapCodec<FluidValue> CODEC = RecordCodecBuilder.mapCodec(
