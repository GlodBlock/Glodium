...
public class RegistryHandler {
    ...
    protected void onRegisterBlocks() {
        for (Pair<String, Block> entry : blocks) {
            ...
            Registry.register(BuiltInRegistries.BLOCK, Glodium.id(this.id, key), block);
        }
    }
    ...
    protected void onRegisterItems() {
        for (Pair<String, Block> entry : blocks) {
            if (this.itemBlocks.containsKey(entry.getLeft())) {
                Registry.register(BuiltInRegistries.ITEM, Glodium.id(this.id, entry.getLeft()), this.itemBlocks.get(entry.getLeft()).apply(entry.getRight()));
            } else {
                Registry.register(BuiltInRegistries.ITEM, Glodium.id(this.id, entry.getLeft()), new BlockItem(entry.getRight(), new Item.Properties()));
            }
        }
        for (Pair<String, Item> entry : items) {
            Registry.register(BuiltInRegistries.ITEM, Glodium.id(this.id, entry.getLeft()), entry.getRight());
        }
    }

    protected void onRegisterTileEntities() {
        for (Pair<String, BlockEntityType<?>> entry : tiles) {
            Registry.register(BuiltInRegistries.BLOCK_ENTITY_TYPE, Glodium.id(this.id, entry.getLeft()), entry.getRight());
        }
    }
